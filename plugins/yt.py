...
def yttt(app,message):
    ...
    if db.get(f"lock_yt_{message.chat.id}"):
        ...
        message.reply_audio(audio=audio_file,performer=channel,title=title,duration=info_dict['duration'],caption=f"@Almortagel20_bot ~ {stm(info_dict['duration'])}")
...
download = InlineKeyboardMarkup (
  [[
    InlineKeyboardButton("المرتجل ♪ ", url='T.me/Almortagel20_bot')
  ]]
)
...
error = InlineKeyboardMarkup (
  [[
    InlineKeyboardButton("⚠️", url='T.me/Almortagel20_bot')
  ]]
)
